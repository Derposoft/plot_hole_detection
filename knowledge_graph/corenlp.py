...
class StanfordCoreNLP:
    ...
    def close(self):
        ...
        if hasattr(self, "p"):
            ...
            logging.info(
                "Killing shell pid: {}, cmdline: {}".format(
    ...
    def tregex(self, sentence, pattern):
        ...
        r_dict = self._request(
            tregex_url, pattern, "tokenize,ssplit,depparse,parse", sentence
        )
        return r_dict
    ...
    def tokensregex(self, sentence, pattern):
        ...
        r_dict = self._request(
            tokensregex_url, pattern, "tokenize,ssplit,depparse", sentence
        )
        return r_dict
    ...
    def semgrex(self, sentence, pattern):
        ...
        r_dict = self._request(
            semgrex_url, pattern, "tokenize,ssplit,depparse", sentence
        )
        return r_dict
    ...
    def word_tokenize(self, sentence, span=False):
        ...
        tokens = [
            token["originalText"] for s in r_dict["sentences"] for token in s["tokens"]
        ]
    ...
    def _request(self, annotators=None, data=None, *args, **kwargs):
        ...
        logging.info(params)
        r = requests.post(
            self.url, params=params, data=data, headers={"Connection": "close"}
        )
    ...
    def _check_args(self):
        ...
        if not re.match("\dg", self.memory):
            raise ValueError(
                "memory=" + self.memory + " not supported. Use 4g, 6g, 8g and etc. "
            )
    ...
    def _check_language(self, lang):
        if lang not in ["en", "zh", "ar", "fr", "de", "es"]:
            raise ValueError(
                "lang="
                + self.lang
                + " not supported. Use English(en), Chinese(zh), Arabic(ar), "
